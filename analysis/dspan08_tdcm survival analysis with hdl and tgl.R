...
for (i in 1:length(analytic_dfs)) {
  ...
  tdcm_df <- df %>%
    ...
    mutate(across(c(bmi, hba1c, homa2b, homa2ir, ldlc, hdlc, tgl, sbp, egfr_ckdepi_2021), ~replace(., is.infinite(.), NA))) %>% 
  ...
  overall_tdcm[[i]] <- coxph(Surv(tstart, tstop, event_true) ~ study + female + race + earliest_age + bmi + hba1c + homa2b_scaled 
                             + homa2ir + ldlc_scaled + hdlc_scaled + tgl_scaled + sbp_scaled + egfr_ckdepi_2021_scaled + dpp_intervention, 
  ...
  sird_tdcm[[i]] <- coxph(Surv(tstart, tstop, sird) ~ study + female + race + earliest_age + bmi + hba1c + homa2b_scaled 
                          + homa2ir + ldlc_scaled + hdlc_scaled + tgl_scaled + sbp_scaled + egfr_ckdepi_2021_scaled, 
                          data = tdcm_df, cluster = joint_id)
...
tdcm_results <- bind_rows(
  ...
  write_csv(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspan08_pooled tdcm results with hdl and tgl.csv"))
