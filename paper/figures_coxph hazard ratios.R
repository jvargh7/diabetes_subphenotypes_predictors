...
coxph_coef <- read_csv(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspan05_pooled cox ph results.csv")) %>% 
...
plot_forest <- ggplot(coxph_coef, aes(y = term, x = estimate, xmin = lci, xmax = uci, color = model)) + 
...
ggsave(plot_forest,filename=paste0(path_diabetes_subphenotypes_predictors_folder,"/figures/coxph hazard ratio for pathophysiological markers.png"),width=10,height=13)
