...
tdcm_coef <- read_csv(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspan08_pooled tdcm results with hdl and tgl.csv")) %>% 
  ...
  dplyr::filter(iv %in% c("bmi","egfr_ckdepi_2021_scaled","hba1c","homa2b_scaled","homa2ir",
                          "ldlc_scaled","hdlc_scaled","tgl_scaled","sbp_scaled")) %>% 
  ...
  term = factor(term,
                levels = c("eGFR", "LDL", "HDL", "TG", "SBP", "HOMA2-IR", "HOMA2-%B", "HbA1c", "BMI"),
...
ggsave(plot_forest,filename=paste0(path_diabetes_subphenotypes_predictors_folder,"/figures/tdcm hazard ratio for pathophysiological markers with hdl and tgl.png"),width=10,height=15.5)
