...
for(i in 1:cardia_mi_dfs$m) {
  cardia_df <- complete(cardia_mi_dfs, action = i) 
  dppos_df <- complete(dppos_mi_dfs, action = i) 
  jhs_df <- complete(jhs_mi_dfs, action = i) 
  mesa_df <- complete(mesa_mi_dfs, action = i) 
...
write_csv(unpooled_resuts, paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspse02b_unpooled tdcm results.csv"))
...
tdcm_results <- bind_rows(
  ...
  write_csv(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspse02b_pooled tdcm results.csv"))
