...
ipw_df <- readRDS(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspse01a_ipw df.RDS")) %>% 
...
for(i in 1:mi_dfs$m) {
  df <- complete(mi_dfs, action = i) %>% 
    ...
    left_join(ipw_df, 
              by = c("joint_id")) %>% 
    dplyr::filter(joint_id %in% ipw_df$joint_id)
...
for (i in 1:length(analytic_dfs)) {
  ...
  overall_tdcm[[i]] <- coxph(Surv(tstart, tstop, event_true) ~ study + female + race + earliest_age + bmi + hba1c + homa2b_scaled 
                             ...
                             data = tdcm_df, cluster = joint_id, weights = ipw_stabilized)
...
write_csv(unpooled_resuts, paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspse01b_unpooled tdcm results with ipw.csv"))
...
tdcm_results <- bind_rows(
  ...
  write_csv(paste0(path_diabetes_subphenotypes_predictors_folder,"/working/processed/dspse01b_pooled tdcm results with ipw.csv"))
